    ...
    class Compiler
      ...
      def create_multiline_arg(pickle_step, _uri)
        if pickle_step[:argument]
          if pickle_step[:argument][:docString]
            doc_string = pickle_step[:argument][:docString]
            Test::DocString.new(
              doc_string[:content],
              doc_string[:media_type]
            )
          elsif pickle_step[:argument][:dataTable]
            data_table = pickle_step[:argument][:dataTable]
            Test::DataTable.new(
      ...
      def source_lines_for_pickle(pickle)
        pickle[:astNodeIds].map { |id| source_line(id) }
      end

      def source_lines_for_pickle_step(pickle_step)
        pickle_step[:astNodeIds].map { |id| source_line(id) }
      end

      def source_line_for_pickle_tag(tag)
        source_line(tag[:astNodeId])
      end
