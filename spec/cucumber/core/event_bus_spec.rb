...
describe Cucumber::Core::EventBus do
  subject(:event_bus) { described_class.new(registry) }

  let(:registry) { { test_event: test_event, another_test_event: another_test_event } }
  let(:test_event) do
    Class.new(Cucumber::Core::Event::Base) do
      def self.event_id
        :test_event
      end
  ...
  context 'when broadcasting events' do
    it 'can broadcast by calling a method named after the event ID' do
      ...
      event_bus.test_event(:redundant_value)
    ...
    it 'can broadcast by calling the `broadcast` method with an instance of the event type' do
      called = false
      event_bus.on(:test_event) { called = true }
      event_bus.broadcast(test_event.new(:some_attribute))
    ...
    describe '#broadcast method' do
      it 'must be passed an instance of a registered event type' do
        expect { event_bus.broadcast(unregistered_event) }.to raise_error(ArgumentError)
      end
    end
  end
  ...
  it 'lets you inspect the registry' do
    expect(event_bus.event_types[:test_event]).to eq(test_event)
  end
